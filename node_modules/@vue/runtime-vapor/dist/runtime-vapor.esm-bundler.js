...
const VaporKeepAlive = /* @__PURE__ */ defineVaporComponent({
	...
	setup(props, { slots, expose }) {
		...
		const cacheBlock = () => {
			...
			innerCacheBlock(resolveCacheKeyFromBlock(innerBlock, interop, isDynamicFragment(block) && block.keyed ? block.current : void 0), innerBlock);
		};
...
const vaporInteropImpl = {
	mount(vnode, container, anchor, parentComponent, parentSuspense, onBeforeMount) {
		...
		if (vnodeBeforeMountHook) callWithAsyncErrorHandling(vnodeBeforeMountHook, parentComponent, 7, [vnode]);
		mountComponent(instance, container, selfAnchor);
		queuePostFlushCb(() => {
			const vnodeHook = vnode.props && vnode.props.onVnodeMounted;
			if (vnodeHook) callWithAsyncErrorHandling(vnodeHook, parentComponent, 7, [vnode]);
		});
	...
	unmount(vnode, doRemove) {
		...
		if (vnodeHook) queuePostFlushCb(() => {
			callWithAsyncErrorHandling(vnodeHook, instance && instance.parent, 7, [vnode]);
		});
	},
	...
	activate(vnode, container, anchor, parentComponent) {
		...
		queuePostFlushCb(() => {
			...
			if (vnodeMountedHook) callWithAsyncErrorHandling(vnodeMountedHook, parentComponent, 7, [vnode]);
		});
	},
...
var TeleportFragment = class extends VaporFragment {
	constructor(props, slots) {
		...
		this.hydrate = () => {
			...
			else this.anchor = locateTeleportEndAnchor(currentHydrationNode.nextSibling);
	...
	handlePropsUpdate() {
		...
		} else if (isTeleportDeferred(this.resolvedProps) || !this.parent.isConnected) {
